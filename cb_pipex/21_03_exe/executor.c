...
*/
int ft_pipe_first(t_exec *node, int pipe_fd[2])
{
	// char *cmd[2];
...
int	ft_executor(t_exec *node)
{
	...
	while(node)
	{
		if (node->next && node->prev)
		{
			...
			pipe(pipe_fd);
			exit_status = ft_pipe_middle(node, pipe_fd, old_pipe_in);
		}
		else if(node->next)
		{
			write(2, "\nentering first pipe\n", 21);
			pipe(pipe_fd);
			exit_status = ft_pipe_first(node, pipe_fd);
			printf("entering first pipe cmd is %s\n", node->cmd[0]);
		}
		else
		{
			write(2, "\nentering last pipe\n", 21);
			exit_status = ft_pipe_last(node, pipe_fd, old_pipe_in);
